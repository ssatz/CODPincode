...
class CODPincode extends PaymentModuleCore {
    ...
    public function hookPayment($params) {
        ...
        $cod_fee =$cart_total_paid<=Configuration::get('COD_MINIMUM_AMOUNT') ? (float)Configuration::get('COD_FEE'):0;
        $smarty->assign(array(
            'this_path' => $this->_path, //keep for retro compat
            'this_path_cod' => $this->_path,
            'this_cod_fee'=>$cod_fee,
		...
		$secure_key = false, Shop $shop = null)
	{
		...
		if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false)
		{
			...
			$cart_total_paid = (float)Tools::ps_round((float)$this->context->cart->getOrderTotal(true, Cart::BOTH), 2);
                        $cart_total_paid =$cart_total_paid<=Configuration::get('COD_MINIMUM_AMOUNT') ? $cart_total_paid + (float)Configuration::get('COD_FEE'):$cart_total_paid;
			...
			foreach ($package_list as $id_address => $packageByAddress)
				foreach ($packageByAddress as $id_package => $package)
				{
					...
					$order->total_discounts = $order->total_discounts_tax_incl;
                                         $cod= (float)Tools::ps_round((float)$this->context->cart->getOrderTotal(true, Cart::BOTH), 2)<=Configuration::get('COD_MINIMUM_AMOUNT') ?  (float)Configuration::get('COD_FEE'):(float)0;
					$order->total_shipping_tax_excl = (float)$this->context->cart->getPackageShippingCost((int)$id_carrier, false, null, $order->product_list)+$cod;
					$order->total_shipping_tax_incl = (float)$this->context->cart->getPackageShippingCost((int)$id_carrier, true, null, $order->product_list)+$cod;
    ...
    public function getContent() {
        ...
        if (Tools::isSubmit('submitShipping')) {
            ...
            if (isset($cod_min) && isset($cod_fee) && Validate::isFloat((float) $cod_min) && Validate::isFloat((float) $cod_fee)) {
    ...
    private function renderForm() {
        ...
        $fields_form_1 = array(
            'form' => array(
                ...
                'description' => $this->l('Define Your COD Setting Here!') . '<br />',
                'input' => array(
                    array(
                        'type' => 'text',
                        'label' => $this->l('COD Minimum Amount'),
                        'name' => 'COD_MINIMUM_AMOUNT',
                    ),
                    array(
                        'type' => 'text',
                        'label' => $this->l('COD Fee'),
                        'name' => 'COD_FEE',
                    )
                ),
                'submit' => array(
    ...
    private function getConfigFieldsValues() {
        return array(
            ...
            'COD_MINIMUM_AMOUNT' => Tools::getValue('COD_MINIMUM_AMOUNT', Configuration::get('COD_MINIMUM_AMOUNT')),
